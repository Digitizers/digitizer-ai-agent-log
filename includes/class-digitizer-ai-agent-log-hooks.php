...
class Digitizer_AI_Agent_Log_Hooks {
	...
	public static function flush() {
		...
		foreach ( $grouped as $blog_id => $blog_rows ) {
			...
			try {
				// Whether a site records is decided per site, and this
				// request only ever ran the check for the site it started on.
				// Now that rows are written in other sites' contexts, that one
				// answer is the wrong one for every other group: a run that
				// switches into a site where the plugin is not active would
				// record there anyway, into a table install_table() was never
				...
				if ( self::blog_records() ) {
					...
					foreach ( $blog_rows as $row ) {
						 ...
						 *                      app, user_id.
						 */
						if ( ! apply_filters( 'digitizer_ai_agent_log_record', true, $row ) ) {
							continue;
						}
	 ...
	 *
	 * The question is whether this site has a table to write to, asked from
	 * that site's own options, so this is only ever meaningful inside the
	 * context the rows are about.
	 *
	 * The schema stamp answers it, and answers activation with it. On a
	 * network the plugin can be active on one site and not another, and
	 * install_table() runs from boot(), which only ever runs on the site the
	 * request started on - so a site reached by switch_to_blog() may have no
	 * table, and inserting into a table that does not exist is an error per
	 * row. The stamp is exactly the right thing to ask, because
	 * install_table() only writes it once it has confirmed the table is
	 * really present. A site the plugin never ran on has no stamp, which is
